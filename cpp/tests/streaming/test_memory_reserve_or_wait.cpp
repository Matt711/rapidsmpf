...
TEST_P(StreamingMemoryReserveOrWait, ShutdownEarly) {
    ...
    std::thread thd([actors = std::move(actors)]() mutable { run_actor_network(std::move(actors)); });
...
TEST_P(StreamingMemoryReserveOrWait, RestartPeriodicTask) {
    ...
    std::thread thd1([actors1 = std::move(actors1)]() mutable { run_actor_network(std::move(actors1)); });
    while (mrow.size() < 1) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    set_mem_avail(20);
    ...
    std::thread thd2([actors2 = std::move(actors2)]() mutable { run_actor_network(std::move(actors2)); });
    while (mrow.size() < 1) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    set_mem_avail(20);
